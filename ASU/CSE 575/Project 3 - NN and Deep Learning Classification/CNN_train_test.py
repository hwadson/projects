...
"""
print('Loading data......')
sub_train_images, sub_train_labels, sub_test_images, sub_test_labels = init_subset('9314') # input your ASUID last 4 digits here to generate the subset samples of MNIST for training and testing
...
def evaluate(net, images, labels):
    ...
    for batch_index in range(0, images.shape[0], batch_size):
        ...
        for l in range(net.lay_num):
            output = net.layers[l].forward(batch_images)
            batch_images=output
        loss += cross_entropy(output, batch_labels)
        if np.argmax(output) == np.argmax(batch_labels):
            acc += 1
    accuracy = acc/images.shape[0]
    losses= loss/images.shape[0]
...
epochs = range(len(train_acc_arr))
plt.plot(epochs, train_acc_arr, 'bo', label='Training accuracy')
plt.plot(epochs, test_acc_arr, 'b', label='Test accuracy', color='red')
plt.title('Model accuracy')
plt.legend()
plt.figure()
plt.plot(epochs, train_loss_arr, 'bo', label='Training loss')
plt.plot(epochs, test_loss_arr, 'b', label='Test loss', color='red')
